...
const readOriginal = (file) => new Promise((resolve, reject) => {
  if (file.size > MAX_UNCOMPRESSED_UPLOAD_SIZE) {
    reject(new Error('Ảnh HEIC/HEIF này quá lớn để tải trực tiếp. Vui lòng chọn ảnh nhỏ hơn 9 MB.'));
    return;
  }
  ...
  reader.onload = () => {
    ...
    resolve(value.replace(/^data:[^;]*;/, `data:${inferredMimeType(file)};`));
  };
...
export async function prepareUploadImage(file) {
  ...
  try {
    return await resizeWithCanvas(file);
  } catch {
    return readOriginal(file);
  }
}
